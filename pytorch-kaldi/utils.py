...
def model_init(inp_out_dict, model, config, arch_dict, use_cuda, multi_gpu, to_do):
    ...
    # At the end inp_out_dict looks like:
    #{'mfcc': ['mfcc', 'exp/UPM_sandbox/exp_files/train_UPM_tr_ep00_ck0_mfcc.lst', 
    #       'apply-cmvn --utt2spk=ark:/home/paul/upm_exp/sa_only/data/train/utt2spk  
...
def forward_model(
    ...
    for line in model:
        ...
        if operation == "compute":
            ...
            if to_do == "forward" and out_name == forward_outs[-1]:
                break
        # lab_dict: {'lab_mono': ['lab_mono', '/home/paul/upm_exp/new_exp/tri1_ali_train', 'ali-to-phones --per-frame=true', 69], 
        ...
        if operation == "cost_nll":
            ...
            # inp = tensor([[-2.4205e+00, -1.8418e+00,  1.2407e+00,  ...,  2.5136e-01,
                    # [ 8.4501e-01,  7.0457e-01, -1.9166e-01,  ..., -9.3894e-02,
                    #   6.0000e+00,  7.3300e+02]], device='cuda:0')
            # torch.Size([128, 71])

            # inp2 = lab_mono
            # lab_dnn = tensor([ 0,  0, 61, 80, 12, 56, 27, 10,  0,  0,  0,  0, 59, 59,  0, 30, 78, 48,...
            # 15,  6], device='cuda:0')
            # torch.Size([128])
...
def dump_epoch_results(res_file_path, ep, tr_data_lst, tr_loss_tot, tr_error_tot, tot_time, 
    valid_data_lst, valid_peformance_dict, lr, N_ep, best_valid_loss, pt_files, model_files, out_folder):
    ...
    # If a new best validation loss, save that model
    valid_loss = valid_peformance_dict[valid_data][0]
    if valid_loss < best_valid_loss:
        print("New best validation loss. Saving model\n")
        best_valid_loss = valid_loss

        # Copy the .pkl files to best_arch.pkl
        for pt_arch in pt_files.keys():
            model_dir = os.path.join(out_folder, "model")
