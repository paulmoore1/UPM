...
for ep in range(N_ep):
    ...
    best_valid_loss = dump_epoch_results(
        res_file_path,
        ep,
        tr_data_lst,
        tr_loss_tot,
        ...
        valid_peformance_dict,
        lr,
        N_ep, best_valid_loss, pt_files, model_files, out_folder)
            ...
            if _run_forwarding_in_subprocesses(config):
                ...
                p = multiprocessing.Process(
                    ...
                    kwargs={
                        ...
                        "articulatory_feats": True,
                        "articulatory_feat_dim": articulatory_feat_dim
                    },
                )
                processes.append(p)
...
if articulatory_feats:
    write_folder = os.path.join(out_folder, pred_folder)
    lab_lock_path = os.path.join(write_folder, "lab_lock.txt")
    if is_universal:
        pred_lock_path = os.path.join(write_folder, "pred_uni_lock.txt")
    else:
        pred_lock_path = os.path.join(write_folder, "pred_lock.txt")
