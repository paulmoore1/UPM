...
# Adapted by Paul Moore to work when the files are only .adc
...
# Checking for sox. Since 'errexit' option is set, the script will
# terminate if sox is not found.
which sox > /dev/null
...
while read line; do
  [[ "$line" =~ ^.*/.*\.adc$ ]] || { echo "Bad line: '$line'"; exit 1; }
  set +e  # Don't want script to die if conversion fails.
  b=`basename $line .adc`; 
  sox -t raw -r 16000 -e signed-integer -b 16 $line \
    -t wav $ODIR/${b}.wav
  if [ $? -ne 0 ]; then
