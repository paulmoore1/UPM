...
for n in $(seq $nj); do
  ...
  utils/create_data_link.pl $featdir/norm_feats_${name}.${n}.ark
done

cp $data_in/utt2spk $data_out/utt2spk
cp $data_in/spk2utt $data_out/spk2utt
cp $data_in/utt2len $data_out/utt2len
...
$cmd JOB=1:$nj $dir/log/norm_feats_${name}.JOB.log \
  apply-cmvn-sliding --norm-vars=$norm_vars --center=true --cmn-window=$cmn_window \
  scp:${sdata_in}/JOB/feats.scp ark:- \| \
  copy-feats --compress=$compress $write_num_frames_opt ark:-\
  ark,scp:$featdir/norm_feats_${name}.JOB.ark,$featdir/norm_feats_${name}.JOB.scp || exit 1;

# Replaced line 70 - originally was:
# copy-feats --compress=$compress $write_num_frames_opt ark:- \

for n in $(seq $nj); do
  cat $featdir/norm_feats_${name}.$n.scp || exit 1;
...
echo "$0: Succeeded applying CMVN features for $name"
